...
- For input validation, I have just validated the **sensor_id** field. But, in future more robust validastion can be done for temperature, humidity and wind_speed to have logical values.
- In the sample data, the values that I have provided are very random, so please don't consider them as logical.
- We can have separate table for just storing all the data related to sensors and then use sensor_id as foereign key in the sensor_metrics table. As of now, I am not storing any sensor related data anyhwere.
